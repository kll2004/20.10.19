...
public class 예외처리1 {
	...
	/* 두 정수와 산술 연산자가 주어지면 주어진 산술연산결과를 알려주는 메소드
	 * 매개변수 : 두 정수,산술연산자
	 * 리턴타입 : 산술연산결과=>실수(나누기)
	 * 메소드명 :calc
	 * calc 메소드에 staric을 붙이는 이유 : 안붙이면 객체 멤버 메소드가 되는데,
	 * 객체 멤버 메소드는 클래스 멤버 메소드인 main에서 호출할 수 없기 때문에
	 * static을 붙여서 클래스 멤버 메소드로 만들어야 한다.
	 * 
	 * 메소드에서 throw를 하면 메소드 선언부에 throws를 이용하여 발생할 수 있는 예외를 적어주어야 한다.
	 * 단, RuntimeException을 상속받은 클래스는 제외
	 *  */
//	public static 리턴타입 매개변수명(매개변수) {}
