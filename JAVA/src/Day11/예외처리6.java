...
public class 예외처리6 {
	...
	/* 기능 : 배열의 길이가 주어지면 주어진 배열의 길이만큼 배열을 생성하여 값을 랜덤으로 초기화 하여 배열을 반환하는 메소드
	 * 		단, 배열의 길이는 0보다 크거나 같다.0보다 작으면 예외발생
	 * 매개변수 : 배열의 길이 =>int num1, 랜덤한범위 random
	 * 리턴타입 : 배열 []
	...
	/* 기능 : 배열이 주어지면 주어진 배열에 모든 값을 랜덤으로 초기화 하는 메소드
	 * 매개변수 : 배열 랜덤범위 = > int []arr, int min, int max
	 * 리턴타입 : 없음 => void
	 * 메소드명 : createRandomArray2
	 * */
	public static int createRandomArray2(int []arr, int min, int max) throws Exception {
		if(arr == null) {
