...
public class 클래스1 {
	...
	public static void main(String[] args) {
		 ...
		 *  - 속성 : 크기, 제조사, 흑백/칼라
		 *  - 기능 : 인쇄, 복사, 스캔
		 *  - 멤버변수의위치는 메소드 밖, 클래스 안
		 *  - 멤버 변수는 클래스 인에서 모든 메소드에서 사용할 수 있다.
		 *  
		 *  - 객체 선언
		 *  클래스명 객체명;
		 *  
		 ...
		 *  객체.메소드명();
		 *  
		 *  - 같은 클래스 안에있는 메소드를 호출할 떄(실행할 떄)는 그냥 사용가능하고
		 ...
		 * 
		 * -public
		 *  - 멤버 메소드와 멤버 변수에 붙일 때는 크게 문제없이 사용 가능
		...
		//실행되는 클래스는 클래스1이라는 다른 클래스이어서 객체를 이용하여 호출해야 한다.
...
class point{
	...
	/* 기능 : x,y좌표 출력 메소드
	 * 매개변수 : 없음 - x,y,좌표는 point 클래스가 스스로 알고 있기 때문에 다른 곳에서 알려줄 필요가 없다.
	 *          다른 곳에서 알려줄 필요가 없다. 
	 * 리턴타입 : 없음
	 * 메소드명 : print
	 * */
