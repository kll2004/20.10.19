...
>>>>>>> main
			...
			case 2:
				...
				case 1:
					System.out.println("-------------------------");
					System.out.println("조회하려는 게시글 번호를 입력");
					System.out.println("-------------------------");
					System.out.print("번호 : ");
					num = scan.nextInt();
				...
				case 4:
					...
					default:
						System.out.println("-------------------------");
						System.out.println("잘못된 조회 방법.");
						System.out.println("-------------------------");
					break;
				}
			case 3:
				//게시글 수정 기능 수행
				//게시글 목록을 열어보고 게시글 수정목록확인
				//게시글 수정사항 확인 제목, 내용
				//수정할게없으면 : 수정하려는 목록과 일치하지않다 출력
				//수정목록 유 : 게시글이 수정되었다. 출력
				
				//수정할 게시글 번호를 입력
				System.out.println("-------------------------");
				System.out.println("수정하려는 게시글 번호를 입력");
				System.out.println("-------------------------");
				System.out.print("번호 : ");
				num = scan.nextInt();
			...
			case 4:
				...
				//삭제할 게시글 번호를 입력
				System.out.println("-------------------------");
				System.out.println("삭제하려는 게시글 번호를 입력");
				System.out.println("-------------------------");
				System.out.print("번호 : ");
				num = scan.nextInt();
				//주어진 번호와 일치하는 게시글 삭제
			...
			case 5:
				...
				if(list.size() ==0) {
					System.out.println("-------------------------");
					System.out.println("등록된 게시물이 없다.");
					System.out.println("-------------------------");
				}
				break;
			case 6:
	...
	public static void addBoard(ArrayList<Board> list){
		...
		System.out.println("----------------------");
		System.out.println("새로운 게시글 정보를 입력");
		System.out.println("----------------------");
	...
	public static void printBoardbyNum(ArrayList<Board>list, int num) {
		//Object.equals를 이용하여 리스트에 있는 값과 매개변수의 값을 비교한 후 클래스가 같으면 Board.equals()를 호출
		...
		if(index != -1) {
			...
			System.out.println("-------------------------");
			System.out.println("해당 번호와 일치하는 글이 없다.");
			System.out.println("-------------------------");			
		}
	}
	/*
	...
	public static void deleteBoardByNum(ArrayList<Board>list, int num) {
		...
		//없으면 삭제할 게시물이 없다고 출력
			...
			System.out.println("-------------------------");
			System.out.println("해당 번호와 일치하는 게시글이 없다.");
			System.out.println("-------------------------");
		}
	}
	/*
	...
	public static void modifyBoardByNum(ArrayList<Board>list, int num) {
		...
		//	- 수정할 제목
			System.out.println("----------------------");
			System.out.println("수정한 게시글 정보를 입력");
			System.out.println("----------------------");
			//제목을 입력받아 String 객체에 저장
			System.out.print("제목 : ");
			scan.nextLine();
	...
	public static void printBoard(ArrayList<Board>list, String option) {			
		//확인하고자하는 게시글의 제목을 입력
		System.out.println("-------------------------");
		System.out.println("조회하려는 게시글 "+option+"을 입력");
		System.out.println("-------------------------");
		System.out.print(option+" : ");
		scan.nextLine();//서브메뉴 입력 후 엔터 처리
		String str = scan.nextLine();
		int count =0;
		//반복문을 이용하여 일치하는 게시글이 없을 때까지 반복
		for(Board tmp : list) {
			//일치하는 글이 있으면 글을 출력하고 count 를 증가
		...
		if(count == 0) {
			System.out.println("-------------------------");
			System.out.println("일치하는 게시물이 없다.");
			System.out.println("-------------------------");
		}		
	}
}